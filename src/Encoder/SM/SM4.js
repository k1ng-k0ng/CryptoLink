 ...
 */
export const encrypt_cbc = (plaintext, key, iv, mode = "base64") => {
    if (!check("iv", iv) && !check("key", key)) { return; }

    let encryptRoundKeys = EncryptRoundKeys(key);
    let plainByteArray = plaintext;
    let padded = padding(plainByteArray);
    let blockTimes = padded.length / UINT8_BLOCK;
    let outArray = [];
    // init chain with iv (transform to uint32 block)
    let chainBlock = getChainBlock(iv);
 ...
 * @returns {String} 加密后的字符串
 */
export const encrypt_ecb = (plaintext, key, mode = "base64") => {
 ...
 */
export const decrypt_cbc = (ciphertext, key, iv, mode = "base64") => {
    if (!check("iv", iv) && !check("key", key)) { return; }
    // get cipher byte array
    let cipherByteArray = null;
    let decryptRoundKeys = EncryptRoundKeys(key).reverse();
    if (mode === 'base64') {
 ...
 * @returns {String} 解密后的字符串
 */
export const decrypt_ecb = (ciphertext, key, mode = "base64") => {
    if (!check("key", key)) { return; }
    ...
    if (mode === 'base64') {
        // cipher is base64 string
        // cipherByteArray = hexToBytes(ciphertext.replace(/\s+/g,""));
		// cipherByteArray = hexToBytes(bytesToString(ciphertext));		
		cipherByteArray = ciphertext;
    } else {
        // cipher is text
        cipherByteArray = stringToArray(ciphertext);
...
// console.log(encrypt_ecb("1234", "1234567890123456"))
